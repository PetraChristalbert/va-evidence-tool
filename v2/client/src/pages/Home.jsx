...
export default function Home() {
    ...
    try {
      ...
      if (data.success) {
        ...
        const interval = setInterval(async () => {
          const statusRes = await fetch(`http://localhost:5000/api/status/${data.jobId}?type=extract`);
  ...
  const handleCancel = async () => {
    try {
      await fetch(`http://localhost:5000/api/cancel/${jobId}`, {
        method: 'POST'
      });
      setStep('confirm');
      setProgressMsg('');
  ...
  const handleContinue = async () => {
    try {
      await fetch(`http://localhost:5000/api/continue/${jobId}`, {
        method: 'POST'
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (step === 'processing') {
      interval = setInterval(async () => {
        try {
          const res = await fetch(`http://localhost:5000/api/status/${jobId}?type=process-pdf`);
          const data = await res.json();
          ...
          <div className="flex flex-col gap-2">
            {Object.keys(packets).map(cond => (
                <div key={cond} className="bg-slate-50 rounded-lg p-3 text-slate-700 flex justify-between items-center">
                    <span>{cond}</span>
                    <a href={`http://localhost:5000/api/download/${packets[cond].split('/').pop()}`} target="_blank" className="text-sky-600 hover:text-sky-700 font-medium">Download</a>
                </div>
            ))}
          </div>
        </section>
